# ── Single-Omics Step: Results & Report ───────────────────────────────────────

#' @noRd
mod_so_report_ui <- function(id) {
  ns <- NS(id)
  step_page(
    .badge_label = "Single-Omics  •  Step 7",
    .title       = "Results & Report",
    .subtitle    = paste0(
      "Download your results as an HTML report or export the ",
      "annotated modules as a table."
    ),
...
#' @noRd
mod_so_report_server <- function(id, annotated_modules, so_data, mo_data, mode,
                              go_back) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$btn_generate, {
      ...
      tmp_dir <- tempfile(pattern = "mapa_report_")
      ...
      promises::future_promise({
        ...
        file.path(tmp_dir, "MAPA_report", "index.html")
      }) |>
        promises::then(
          function(html_file) {
    ...
    output$download_report <- downloadHandler(
      filename = function() paste0("mapa_report_", Sys.Date(), ".zip"),
      content  = function(file) {
        dir <- req(report_dir())
        owd <- setwd(dir)
        on.exit(setwd(owd), add = TRUE)
        utils::zip(file, files = "MAPA_report")
      }
    )
