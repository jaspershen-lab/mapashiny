...
.input_format_popover <- function(query_type,
  ...
  bslib::popover(
    trigger = tags$button(
      ...
      # Keep the essential appearance inline so that the trigger cannot fall
      # back to the browser/Bootstrap default while a cached stylesheet is
      # being refreshed. Hover and mode-specific colours remain in custom.css.
      style = paste(
